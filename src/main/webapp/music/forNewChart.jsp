...
<article class="allchart">
	<div class="topchart">
		...
		<span style="font-size:30px; font-weight:bold;">최신음악</span><span style="font-size:25px; font-weight:bold; color:#4C4C4C;">(해외)</span>
		&nbsp;&nbsp;&nbsp;
		<button onclick="location.href='IndieServlet?command=AllNewest'">종합</button>
		<button onclick="location.href='IndieServlet?command=KorNewest'">국내</button>
		<button onclick="location.href='IndieServlet?command=ForNewest'">해외</button>
	</div>
	<table class="chart">
	<div class="hide" style="display: none;">
		<form name="onoff" class="formm" action="IndieServlet?command=ForNewest" method="post">
		...
		<c:forEach items="${getForNewest}" var="chartList" varStatus="status" begin="0" end="14" step="1">
			<tr class="line">
...
<div class="mbtn">
		...
		<table class="chart">
			<c:forEach items="${getForNewest}" var="chartList" begin="15" varStatus="status">
			<tr class="line">
